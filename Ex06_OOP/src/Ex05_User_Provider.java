...
// 2. 포함(has ~ a) : 경찰은 권총을 가지고 있다. member field 형태로 A라는 클래스 안에 B라는 클래스가 들어오는 것
					//class A { B b; }
// 3. 포함(has ~ a) : 함수 안에서(객체 생성, 할당, 리턴) dependency 의존관계
// 연관관계
// 학교는 학생을 가지고 있다. 집합연관 class school { Student st; }, class Student{ }
// 자동차는 엔진을 가지고 있다. 혹합연관 class Car { Engine en = new Engine() }
...
//간접적인 기법(유연하다)
class C{
	void method(Icall ic) {	//핵심! 한 줄의 코드
		ic.m();
	}
}
