...
class Scv extends GroundUnit implements Irepairable{
	...
	*/
	...
	//고민은 : 하나의 함수로 다른 모드 unit을 수리할 수 없을까?
	//1. 제시 : void repair(GroundUnit Unit) ... Marine2 (repair X)
	
	//2. 제시 : void repair(Irepairable repairunit)
	//		   Irepairable 부모타입 ----> 자식타입 Scv, CommandCenter, Tank2
	//   고민 : repairunit 참조변수가 접근할 수 있는 자원이 없음 
	...
	void repair(Irepairable repairunit) {
		...
		//down casting
			...
			if(unit.hitpoint != unit.MAX_HP) {
				...
				//CommnadCenter 객체의 주소
				//void repair(Irepairable repairunit)
				System.out.println("unit 아니에요");
			}
