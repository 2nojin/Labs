...
public class Ex01_Array_Basic {
	...
	public static void main(String[] args) {
		...
		//각각의 방에 대한 접근은:index로 접근:배열의 시작값은 0
		//score[0], score[1], score[2]
		//배열의 갯수 (length) 3개
		//배열의 마지막 index값은 : length-1
		//배열은 타입의 default값을 가진다.
		
		//score[3]=500;
		//예외(Exception 발생) >> 현상 >> 프로그램 강제 종료
		//java.lang.ArrayIndexOutOfBoundsException: 3
		//자주 봅시다
		
		
		// Array와 궁합이 제일 좋은 제어문? for문
		for(int i=0; i<3; i++) {
			System.out.printf("score[%d]=%d\t",i,score[i]);
		}
		
		System.out.println();
		for(int i=0; i<score.length; i++) {	// 배열의 갯수 = length
			System.out.printf("score[%d]=%d\t",i,score[i]);
		}
		...
		int[] arr2 = new int[] {100,200,300};	// 초기값을 정의해서 배열생성
		int[] arr3 = {11,12,13,14,15};	// 값만 설정 (컴파일러가 내부적으로 new생성)
		
		//javascript : var cars = ["kia","volvo","bmw"];
